  ...
  return (
    <div className="relative flex items-center justify-center gap-2 sm:gap-4 mb-8">
      ...
      <motion.div
        ...
        className="relative"
      >
        <span className="font-playfair text-8xl sm:text-9xl lg:text-[12rem] font-bold text-gray-900/10 select-none">
          4
        </span>
        <motion.span 
          className="absolute inset-0 font-playfair text-8xl sm:text-9xl lg:text-[12rem] font-bold text-gray-900 select-none"
      ...
      >
        <motion.div
          ...
          className="w-16 h-16 sm:w-24 sm:h-24 lg:w-32 lg:h-32 border border-gray-900/20 flex items-center justify-center"
        >
          <FlowerDecor className="w-8 h-8 sm:w-12 sm:h-12 lg:w-16 lg:h-16 text-gray-900" />
        </motion.div>
        
        {/* Pulsing ring */}
        <motion.div
          className="absolute inset-0 border border-gray-900/10"
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {shapes.map((shape, i) => (
        <motion.div
          key={i}
          className="absolute border border-gray-900/5"
          style={{
            width: shape.size,
            height: shape.size,
            left: `${shape.x}%`,
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden flex items-center justify-center px-4 py-12">
      ...
      <div className="relative z-10 w-full max-w-4xl">
        ...
        >
          ...
          >
            <motion.div
              ...
              className="w-16 h-px bg-gray-900 mx-auto mb-6 origin-center"
            />
          </motion.div>
          ...
          <motion.p
            variants={itemVariants}
            className="text-gray-900/60 max-w-lg mx-auto mb-8 leading-relaxed"
          >
          ...
          >
            <form onSubmit={handleSearch} className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 border border-gray-900/10 focus:border-gray-900 outline-none transition-colors text-gray-900"
              />
              <motion.button
                type="submit"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white px-4 py-2 font-medium hover:bg-gray-800 transition-colors cursor-pointer"
              >
                Search
              </motion.button>
            </form>
          ...
          <motion.div
            ...
            className="mb-10"
          >
            <p className="text-xs tracking-[0.2em] text-gray-900/40 uppercase mb-6">
              Quick Links
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl mx-auto">
              {quickLinks.map((link, index) => (
                ...
                >
                  <Link to={link.path}>
                    <motion.div
                      whileHover={{ y: -5, borderColor: 'rgba(17, 24, 39, 0.3)' }}
                      className="relative bg-white border border-gray-900/10 p-6 text-center group cursor-pointer transition-colors"
                    >

                      <motion.div
                        whileHover={{ scale: 1.1, rotate: 5 }}
                        className="w-10 h-10 border border-gray-900/10 flex items-center justify-center mx-auto mb-3 group-hover:border-gray-900/30 transition-colors"
                      >
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="inline-flex items-center gap-2 px-6 py-3 border border-gray-900 text-gray-900 font-medium hover:bg-gray-900 hover:text-white transition-colors cursor-pointer group"
            >
            ...
            <Link to="/">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 px-6 py-3 bg-gray-900 text-white font-medium hover:bg-gray-800 transition-colors cursor-pointer group"
              >
          ...
          <motion.div
            variants={itemVariants}
            className="flex items-center justify-center gap-3 text-gray-900/40"
          >
            <motion.div
              animate={{ rotate: 360 }}
        ...
        >
          <div className="relative inline-block bg-gray-50 border border-gray-900/10 px-8 py-6">

            <div className="flex items-center justify-center gap-2 mb-2">
              <HelpCircle className="w-4 h-4 text-gray-900/50" />
              <span className="text-sm text-gray-900/50">Need help?</span>
            </div>
      ...
      <motion.div
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 w-32 h-px bg-gray-900/10"
      />
    </div>
  );
};
