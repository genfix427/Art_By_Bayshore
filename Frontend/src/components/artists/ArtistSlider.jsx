// components/artists/ArtistSlider.jsx
...
const ArtistCardSkeleton = ({ index }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
...
const ArtistSlider = ({ 
  ...
  const totalSlides = Math.max(0, Math.ceil(artists.length / slidesPerView));
  ...
  if (error && artists.length === 0 && !loading) {
    return null;
  }
          ...
          <div className="overflow-hidden px-2">
            {loading ? (
              ...
              <AnimatePresence mode="wait" initial={false}>
                ...
                >
                  {getVisibleArtists().map((artist, idx) => (
                    <motion.div
                      key={artist?._id || idx}
                      initial={{ opacity: 0, y: 20 }}
